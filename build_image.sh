...
#docker build --build-arg GRAFANA_VERSION=${_grafana_version} --tag "quiqueporta/graphite_grafana:${_grafana_tag}"  --no-cache=true .

#if [ $_release_build == true ]; then
docker build --build-arg GRAFANA_VERSION=${_grafana_version} --tag "quiqueporta/graphite_grafana:latest"  --no-cache=true .
#fi